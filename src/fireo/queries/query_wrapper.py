...
class ModelWrapper:
    ...
    def from_query_result(cls, model, doc, nested_doc=False):
        ...
        # If it is not nested model then set the id and _reference for this model
        if not nested_doc:
            ...
            # reference is needed to query `Reference` type field
            setattr(model, '_reference', doc.reference)
